...
def number_attracting_components(G, get_chunks="chunks"):
    ...
    of nodes into chunks and then finding the length of each chunk in parallel
    ...
    if get_chunks == "chunks":
        node_iter_chunks = nxp.chunks(attracting_comp_list, n_jobs)
    else:
        node_iter_chunks = get_chunks(attracting_comp_list)

    results = Parallel()(delayed(len)(chunk) for chunk in node_iter_chunks)
    return sum(results)
