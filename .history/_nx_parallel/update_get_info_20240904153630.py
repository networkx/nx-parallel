...
def extract_add_params(docstring):
    """Extract the parameter description from the given docstring."""
    try:
        par_params = {}
        doc_lines = docstring.split("\n")

        # Start searching for the Parameters section
        ...
        for line in doc_lines:
            ...
            if in_params_section:
                if " : " in line:  # indicates the start of a new parameter
                    if current_param:
                        par_params[current_param] = " ".join(param_desc).strip()

                    # split the parameter name and type
                    current_param = line.split(" : ")[0].strip()
                    param_desc = []
                elif current_param:
                    param_desc.append(line)  # Collect description lines
